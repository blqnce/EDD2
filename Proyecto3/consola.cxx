...
void Consola::procesar_comando(const vector<string>& comando) {
    ...
    // CADA COMANDO LLAMA A UN METODO DE FASTA (MANEJA LA LOGICA DE TODO EL PROGRAMA)
    ...
    } else if (nombre == "ruta_mas_corta") {
        if (!fasta_data.esta_cargado()) { 
            cout << "No hay secuencias cargadas en memoria" << endl; 
            return; 
        }
        // Estructura del comando: ruta_mas_corta <desc> <i> <j> <x> <y>
        string desc = comando[1];

        int i = stoi(comando[2]);
        int j = stoi(comando[3]);
        int x = stoi(comando[4]);
        int y = stoi(comando[5]);

        fasta_data.ruta_mas_corta(desc, i, j, x, y);
    //--------------//--------------//--------------//--------------//--------------//--------------//--------------//--------------//--------------//
    // COMANDO BASE_REMOTA 
    } else if (nombre == "base_remota") {
        if (!fasta_data.esta_cargado()) { 
            cout << "No hay secuencias cargadas en memoria" << endl; 
            return; 
        }

        // Estructura del comando: base_remota <desc> <i> <j>
        string desc = comando[1];

        int i = stoi(comando[2]);
        int j = stoi(comando[3]);

        fasta_data.base_remota(desc, i, j);
    }
}